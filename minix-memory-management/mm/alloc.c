...
PRIVATE int is_worst_fit = 0;	/* flag to choose first fit or worst fit */
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *max_hp, *max_prev_ptr;
  ...
  if(is_worst_fit) {
  	do {
  		...
  		hp = hole_head;
      		max_hp = hole_head;
      		max_prev_ptr = NIL_HOLE;
      
      		/* searching for the biggest hole */
      		while (hp != NIL_HOLE && hp->h_base < swap_base) {
        		if(hp->h_len > max_hp->h_len) {
          			max_hp = hp;
          			max_prev_ptr = prev_ptr;
        		}
        		prev_ptr = hp;
        		hp = hp->h_next;
      		}
      		/* checking if hole is good */
      		if(max_hp->h_len >= clicks) {
        		old_base = max_hp->h_base;
        		max_hp->h_base += clicks;
        		max_hp->h_len -= clicks;
        		if(max_hp->h_len == 0) del_slot(max_prev_ptr, max_hp);
        		return(old_base);
      		}
	  } while (swap_out());		
  }
...
PUBLIC int do_hole_map() {
 	register int hole_count = 0;
 	register size_t idx = 0;
	register struct hole* iter = hole_head;
  	register phys_bytes copy_size = 1024 * sizeof(phys_clicks);
  	phys_clicks buffer[1024];

  	while (iter != NIL_HOLE && iter->h_base < swap_base && idx < 1022) {
		buffer[idx++] = iter->h_len;
		buffer[idx++] = iter->h_base;
		hole_count++;
		iter = iter->h_next;
  	}

  	buffer[idx] = 0;
 	copy_size = MIN(copy_size, mm_in.m1_i1);
  	copy_size = MIN(copy_size, idx * sizeof(phys_clicks));

  	memcpy(mm_in.m1_p1, buffer, copy_size);
